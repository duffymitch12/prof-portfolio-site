...
export default function Navbar() {
  ...
  return (
    <nav className="fixed w-full top-0 pt-2 z-50 border-b-2 border-gray-200 pb-6 md:pb-2 bg-[#fefefe]">
      ...
      {!isOpen && (
        <h1 className="md:hidden text-2xl font-semibold text-[#000101] text-opacity-100 ml-4 mt-2 text-nowrap underline underline-offset-[6px] decoration-[#277986] decoration-[3px]">
          Mitchell Duffy
        </h1>
      )}
      {isOpen && (
        <div className="md:hidden fixed grid grid-cols-4 gap-0 text-left transition-transform duration-300 inset-0">
          ...
          <div
            className="col-span-1 h-full w-full backdrop-blur-md"
            onClick={closeMenu}
          >
